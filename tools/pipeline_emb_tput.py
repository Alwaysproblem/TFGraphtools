...
def load_tf_graph(frozen_graph_filename,
                  with_meta_graph=True,
  ...
  if with_meta_graph:
    ...
    with tf.Session(graph=tf.Graph()) as sess:
      meta_graph = tf.saved_model.loader.load(sess, [tag] if tag else [],
                                              model_path)
...
def gen_data(bs, graph):
  ...
  for key, value in data.items():
    ...
    feed_dict[graph.get_tensor_by_name(
        f"TensorDict/StandardKvParser_{idx}:0"
        if idx != 0 else "TensorDict/StandardKvParser:0")] = v

  return feed_dict


def run_tput(model_path,
             bs=1,
             pipline_stages=PIPELINE_STAGE,
             iterations=NUM_ITER):
  ...
  with tf.Session(graph=graph, config=sess_cfg) as session:
    ...
    thp = [
        Thread(target=runner,
               args=(bs, graph, feed_dict, session, sleep_times[i]))
        for i in range(pipline_stages)
    ]
    ...
    (_, idx, counts) = np.unique(latency_list,
                                 return_index=True,
                                 return_counts=True)
...
if __name__ == "__main__":
  ...
  run_tput(args.model_path,
           args.batch_size,
           pipline_stages=args.pipline_stages,
