...
def meta_to_saved_model(meta_model_input_path,
                        savedModel_path,
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      output_node_pl = [
          sess.graph.get_tensor_by_name(f"{t}:0") for t in output_node_names
      ]
      o = sess.run(
          output_node_pl,
          feed_dict={
              ...
              np.random.randint(0, 10,
                                size=(batch_size,
                                      max_sequence_length)).astype(np.int32),
              input_mask_pl:
              np.random.randint(0, 10,
                                size=(batch_size,
                                      max_sequence_length)).astype(np.int32),
      ...
      input_placeholder_info = [
          tf.saved_model.utils.build_tensor_info(
              sess.graph.get_tensor_by_name(i))
          for i in ("input_ids:0", "input_mask:0", "segment_ids:0")
      ]
      output_node_pl_info = [
          tf.saved_model.utils.build_tensor_info(
              sess.graph.get_tensor_by_name(f"{tens}:0"))
          for tens in output_node_names
      ]
      prediction_signature = (
          tf.saved_model.signature_def_utils.build_signature_def(
              inputs={
                  ip.name.split(':')[0]: ip
                  for ip in input_placeholder_info
              },
              outputs={
                  res.name.split(':')[0]: res
                  for res in output_node_pl_info
              },
