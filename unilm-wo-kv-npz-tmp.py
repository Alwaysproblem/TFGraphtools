...
parser.add_argument("-m", dest = "model_path", type=str, action="store", default=".")
...
def gen_data(bs):
  ...
  for i in range(0, len(data["tensordict_standardkvparser_3_args"]), bs):
    yield data["tensordict_standardkvparser_3_args"][i:i + bs], data["tensordict_standardkvparser_2_args"][i:i + bs]


with tf.Session(config=sess_cfg) as sess:
  meta = tf.saved_model.loader.load(sess, ['serve'], args.model_path)
  ...
  for iter_count in range(args.num_iterations):
    ...
    for ids, masks in gen_data(args.batch_size):
    
      start = time.time()
      predictions = sess.run(out_names_pl, feed_dict={
        StandardKvParser_3: ids,
        StandardKvParser_2: masks,
      })
      stop = time.time()
      duration = stop - start
